...
- **灰度变换与增强**: 图像反相、亮度/对比度、对数变换、伽马校正、直方图均衡化、直方图规定化、CLAHE、灰度级分层、位平面分解、图像加法去噪、图像减法差异。
- **空间滤波**: 均值滤波、高斯滤波、中值滤波、双边滤波、拉普拉斯锐化、反锐化掩蔽。
- **边缘检测**: Sobel、Prewitt、Roberts、Kirsch (8方向罗盘)、Laplacian、Canny、Hough 直线检测、Hough 圆检测。
- **阈值与分割**: 手动阈值、Otsu、Triangle、自适应阈值、分水岭分割。
- **形态学**: 腐蚀、膨胀、开运算、闭运算、形态学梯度、顶帽、黑帽，支持矩形、椭圆、十字结构元素。
- **频域处理**: 理想/高斯低通、高通、带通、带阻滤波。
- **噪声与复原**: 高斯噪声、椒盐噪声、运动模糊模拟、运动模糊复原（维纳滤波反卷积）、维纳自适应滤波去噪。
- **颜色图像处理**: B/G/R、灰度、HSV、Lab 通道观察、伪彩色映射、白平衡校正（灰度世界法/完美反射法）。
- **几何处理**: 旋转、水平翻转、垂直翻转、缩放。
- **压缩与质量评价**: JPEG 压缩预览、MSE、PSNR、SSIM。
- **小波与多分辨率** *(新增)*: Haar 2D DWT 小波分解可视化、小波软阈值去噪。
- **图像压缩编码** *(新增)*: 哈夫曼编码统计（熵/平均码长/压缩率）、DPCM 预测编码（四种预测器）。
- **图像表示与描述** *(新增)*: 边界链码提取（4/8方向）、GLCM 纹理特征（Haralick 6特征）、距离变换（D4/D8/欧氏）。
